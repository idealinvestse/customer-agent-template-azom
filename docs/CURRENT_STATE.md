...
| **FAQ/KB v1** | Lexical FAQ corpus (`config/faq/` + `config/faq_synonyms.yaml`), prefix/synonym search, draft enrichment (prompt 1.3 treats FAQ as policy), WP sync-before-publish + category-grouped HTML + drift, CLI `faq` (coverage `gaps_vs_se` / validate / reload / staging purge), dashboard `/faq` + `/faq/<id>` + `/oscar/faq` (staging promote HITL), `kpis` FAQ retrieve/hit per category and market (`faq_retrieve` telemetry). See [`FAQ_KB.md`](FAQ_KB.md). **Live SE FAQ page** is an ops gate — not marked done here. |
| **FAQ ingest** | HITL pipelines: `faq dataset export`, `faq ingest site/products`, `faq suggest articles` (`--from` staging\|dataset\|products\|guides), Oscar `faq promote --apply` (`--force` to overwrite `ingest_{id}.yaml`). Staging under `AZOM_DATA_DIR`; kill-switch `AZOM_FAQ_INGEST_KILL`. GDPR-delete scrubs dataset `case_id` rows; `retention-purge` ages staging files. No silent YAML/WP write. |
